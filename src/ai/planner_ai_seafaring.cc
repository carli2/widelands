...
void PlannerAI::gain_ship(Widelands::Ship& ship, NewShip type) {
...
Widelands::IslandExploreDirection PlannerAI::randomExploreDirection() {
...
// This scores a spot for potential colony.
uint8_t PlannerAI::spot_scoring(Widelands::Coords candidate_spot) {
	...
	if (other_player_accessible(persistent_data->colony_scan_area, &tested_fields,
	                            &mineable_fields_count, candidate_spot)) {
		return 0;
	}
...
// Simplified BFS-based accessibility check for PlannerAI.
// Walks outward from starting_spot using map neighbours and returns true
// if any field owned by *any* player is found within max_distance.
// Also counts tested_fields and mineable_fields_count for the caller.
bool PlannerAI::other_player_accessible(const uint32_t max_distance,
	...
	while (!queue.empty()) {
		...
		// PlannerAI simplified version: any owned territory means another player is accessible
		...
		// obstacles are usually wider than one field
...
bool PlannerAI::marine_main_decisions(const Time& /* gametime */) {
	...
	// Find the port BuildingObserver by attribute
	BuildingObserver* port_bo = nullptr;
	...
	if (port_bo != nullptr) {
		ports_finished_count = port_bo->cnt_built;
		ports_count = ports_finished_count + port_bo->cnt_under_construction;
	}

	// goes over all warehouses (these include ports)
 ...
 */
void PlannerAI::manage_shipyards() {
...
bool PlannerAI::check_ships(const Time& gametime) {
 ...
 */
void PlannerAI::check_ship_in_expedition(ShipObserver& so, const Time& gametime) {
	Widelands::PlayerNumber const pn = player_->player_number();
	...
	if (expedition_ship_ == kNoShip) {
	...
	} else if (expedition_time >= kExpeditionMaxDuration) {
		...
		assert(persistent_data->expedition_start_time >
		       Widelands::Player::AiPersistentState::kNoExpedition);
		assert(expedition_time < kExpeditionMaxDuration);
		...
		const uint8_t remaining_time =
		   100 - ((gametime - persistent_data->expedition_start_time).get() /
		          (kExpeditionMaxDuration.get() / 100));
...
// navigation decisions (these notifications are processed not in 'real time')
void PlannerAI::expedition_management(ShipObserver& so) {
	...
	bool port_allowed = basic_economy_established ||
	                    (port_bo != nullptr &&
	                     port_bo->cnt_built + port_bo->cnt_under_construction < 1);
	if (!port_allowed) {
		game().send_player_cancel_expedition_ship(*so.ship);
		return;
	}
...
// navigation decisions (these notifications are processed not in 'real time')
void PlannerAI::warship_management(ShipObserver& so) {
	const Time& gametime = game().get_gametime();
...
bool PlannerAI::attempt_escape(ShipObserver& so) {
	...
	for (Widelands::Direction dir = Widelands::FIRST_DIRECTION; dir <= Widelands::LAST_DIRECTION;
		...
		for (int8_t i = 0; i < 30; ++i) {
			...
			if (i == 5) {
				// If open sea goes at least 5 fields from the ship this is considered a
...
Widelands::ShipFleet* PlannerAI::get_main_fleet() {
